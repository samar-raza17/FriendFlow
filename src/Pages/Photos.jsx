  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q,
      (snapshot) => {
        ...
        .filter((post) => post.data.hasOwnProperty('image'));
        setData(newPosts);
        setIsLoading(false);
      },
      (error) => {
    ...
    <div className='flex pt-[8vh]'>
      ...
      {isLoading && <Skeleton4Post/>}
       ...
       data?.length > 0 ? data?.map((data, index) => {
        ...
        const doc = data?.data;
        return(
          <Post key={index} image={doc?.image} uploadAt={doc?.uploadAt?.seconds} likes={doc?.likes?.length} uid={doc?.uid} title={doc?.title} docID={id}isLikeBtn={doc?.likes?.includes(authObj?.uid)} isNavigate={true}/>
        )
